...
async function showModal(id) {
  pending.value = true
  const { data: sign } = await useAsyncData(`fetch-marking-${id}`, () => queryContent('theory', 'road-markings', id).findOne())
  pending.value = false
  show.value = true
...
<template>
  <span class="link inline-flex flex-row gap-2 justify-start items-center px-2 bg-base-200 rounded-full hover:bg-base-300" @click="showModal($props.id)">
    {{ $props.id }}
    <UIAvatar size="w-4" :src="`/img/markings/${$props.id}.png`" />
  </span>
  <UIModal v-model:show="show">
    ...
    <div v-else class="flex flex-col w-full ">
      ...
      <div class="flex flex-row justify-start items-center gap-2">
        <UIButton label="More" color="primary" @click="navigateTo(`/theory/road-markings/${$props.id}`)" />
